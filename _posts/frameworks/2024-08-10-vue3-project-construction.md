...
![image.png](/img/frameworks/vue3-template.png)
...
网页上的效果如下：
![image.png](/img/frameworks/vue3-layout.png)
...
![image.png](/img/frameworks/lang-temp.png)
...
- 捕捉需要翻译的内容
  ...
  ![image.png](/img/frameworks/lang-tem1.png)

- 提取文案，生成 key

  ![image.png](/img/frameworks/lang-tem2.png)

- 生成路径

  提取文案后会生成路径 `ni-hao`，这里可以理解为「根路径」，但我是使用模块进行区分的，因此将路径改成 `home.ni-hao`。

  ![image.png](/img/frameworks/lang-tem3.png)

- 替换内容

  这时，可进行内容替换，如是在 `template` 模版中，则使用第一个。

  ![image.png](/img/frameworks/lang-tem4.png)

- 基本语言与翻译语言

  上一步即可生成 key，但是，生成的是 `cn.json`的还是 `en.json`的，需要我们进行配置。

  ![image.png](/img/frameworks/lang-tem5.png)

  如上图，基准语言则为生成 key 的语言，可视语言即为在代码中默认看到的语言。如我配置的都是 `cn`。 看一下效果：

  ![image.png](/img/frameworks/lang-tem6.png)
  ![image.png](/img/frameworks/lang-tem7.png)
  ![image.png](/img/frameworks/lang-tem8.png)

- 翻译情况

  可以在插件面板查看翻译情况，可以看到，那些内容是已经翻译的。 如之前的 「home.ni-hao」，字段只在 `cn.json` 中有，而在 `en.json` 中没有。

  ![image.png](/img/frameworks/lang-tem9.png)

...
| 🌐 **必须联网**   | commit 必须连接服务器，断网基本废了    | ⚠️ 硬伤     |
| 🔒 **权限控制强** | 可精确控制谁能读、谁能写、哪个目录能改 | ✅ 企业友好 |
| ⚠️ **分支很重**   | 创建/切换分支成本高，没有 Git 那么轻量 | ❌ 不灵活   |
...
- ❌ **没有工作区状态**：代码改动了，没有状态区分。A功能完成70%，要优化只能在工作区修改，出问题无法回退到70%节点
- ❌ **没有隔离**：两个功能之间没有隔离，很难控制变更
- ❌ **无法恢复**：AI改坏代码后，无法恢复之前已处理好的代码
...
| 📦 **多需求并行**    | 线上 Bug 改动 + 新需求改动混在一起，不能拉取/推送代码，只能直接打包发布                                  |
...
> ⚠️ **槽点**：使用汉语拼音首字母作为项目名称，明明是 H5，却用了 `mini`。没有 README.md，根本不知道项目做什么。

##### 🗂️ 2. SVN 地址（静态资源 vs HTML 分离）
...
| 🔐 **私有页面**     | `rank` 功能，代码在 `a` 或 `d` 或 `d2` 目录，部署到 d 目录。没有文档说明                                    |

> ⚠️ **最大痛点：源代码目录、部署目录的对应性没有规范**
>
...
很遗憾的是，连让测试规范下bug的描述的建议都没有得到重视，更别说这种部署流程了。
